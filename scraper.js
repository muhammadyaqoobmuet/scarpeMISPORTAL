...
export async function scrapeAttendanceData() {
  ...
  executablePath:
    ...
    await page.waitForSelector("#table-container", { visible: true, timeout: 30000 });
    await page.waitForSelector(".table.sheet.header-fixed", { visible: true, timeout: 30000 });
    ...
    const subjects = [
      "DSS", "DS-A", "DS-A Lab", "MAD", "MAD Lab", "TSW", "SPM"
    ];

    const jsonResult = subjects.map((subjectName, index) => ({
      subjectName,
      conducted: conducted[index] || "0",
      attended: attended[index] || "0",
      missed: (parseInt(conducted[index]) - parseInt(attended[index])).toString() || "0",
